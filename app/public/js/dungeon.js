(async (window, d, undefined) => {
    ...
    async function get_checkpoints() {
        ...
        const checkpoints_regex = {
            ...
            'DreamShaperXLTurboV2':/.*dreamshaper.*xl.*v2.*turbo.*\.safetensors$/gi,
        ...
        console.log(available_checkpoints)
    ...
    roll.addEventListener('click', async (event) => {
        if (IS_GENERATING) {
            ...
            let CFG = 6.5;
            console.log(style_input.options)
    ...
    console.log(protocol, server_address, client_id)
        ...
        if ( data.type === 'progress' ) {
            ...
            if ('images' in data['data']['output']) {
                ...
                for (let i = 0; i < images.length; i++) {
                    ...
                    results.innerHTML += '<div' + grid + '><div><a href="http://localhost:8188/view?filename=' + filename + '&type=output&subfolder=' + subfolder + '&rand=' + rand + '" data-type="image"><img src="http://localhost:8188/view?filename=' + filename + '&type=output&subfolder=' + subfolder + '&rand=' + rand + '" width="1024" height="1024" alt=""></a></div></div>';
                }
            }
